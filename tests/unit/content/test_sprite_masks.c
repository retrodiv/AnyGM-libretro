...
int main(void){
  ...
  frames[1]=gmlc_project_add_memory_file(&project,"narrow",GMLC_MEMORY_RGBA,narrow,sizeof narrow,
                                         SPRITE_SIZE,SPRITE_SIZE);
  REQUIRE(frames[0] && frames[1],"subimage fixtures");

  sprite.id=sprite.name=(char *)"spr_fixture";
  sprite.width=sprite.height=SPRITE_SIZE;
  sprite.bbox_right=sprite.bbox_bottom=SPRITE_SIZE-1;
  sprite.bbox_mode=2;
  sprite.n_frames=2;
  sprite.frame_paths=frames;
  sprite.sep_masks=0;
  ...
  int failures=0;
  if(rendered){
    const GmlSprite *packed=&render.spr[0];
      ...
      for(int y=0;y<SPRITE_SIZE;y++)
        for(int x=0;x<SPRITE_SIZE;x++){
          ...
          if(mask_bit(packed,x,y)!=expected){
            fprintf(stderr,
                    "sprite masks failed: shared mask at %d,%d is %d, the last subimage says %d\n",
