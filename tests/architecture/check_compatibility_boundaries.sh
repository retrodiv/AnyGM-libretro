...
override_owner_definitions=$(find src/core -type f -name '*.c' -exec grep -n -H -E \
  '^[[:space:]]*(static[[:space:]]+)?(int|void)[[:space:]]+(core_opt_(god|start_room)|engine_override_(reset|set)|apply_sticky_cheats|aspect_apply_program|aspect_(compositor_fullwidth|center_view_target|wide_gameplay_view|draw_full_view)_gen|menu_run|room_skip_hook|introskip_hook)[[:space:]]*\(' \
