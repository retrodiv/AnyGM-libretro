...
Wwise_RIFF_Vorbis::Wwise_RIFF_Vorbis(
  ...
  :
    _infile(string(data,data_size),ios::binary),
    _codebook_data(codebook_data),
    _codebook_size(codebook_size),
    _file_size((long)data_size),
    _little_endian(true),
    _riff_size(-1),
    _fmt_offset(-1),
    _cue_offset(-1),
...
void Wwise_RIFF_Vorbis::generate_ogg_header(Bit_oggstream& os, bool * & mode_blockflag, int & mode_bits)
{
    ...
    // generate setup packet
    {
        ...
        else
        {
            ...
            codebook_library cbl(_codebook_data,_codebook_size);
