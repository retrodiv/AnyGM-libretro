...
codebook_library::codebook_library(const char *data,size_t size)
    : codebook_data(NULL), codebook_offsets(NULL), codebook_count(0)
{
    if (!data || size < 4) throw Parse_error_str("codebook library truncated");
    uint32_t offset_offset=read_32_le(
        reinterpret_cast<unsigned char *>(const_cast<char *>(data+size-4)));
    if (offset_offset > size-4 || ((size-offset_offset)&3u))
        throw Parse_error_str("invalid codebook offset table");
    codebook_count=(long)((size-offset_offset)/4u);
    if (codebook_count < 2) throw Parse_error_str("empty codebook offset table");
    codebook_data=new char[offset_offset];
    codebook_offsets=new long[codebook_count];
    memcpy(codebook_data,data,offset_offset);
    for (long index=0;index<codebook_count;index++) {
        uint32_t offset=read_32_le(reinterpret_cast<unsigned char *>(
            const_cast<char *>(data+offset_offset+(size_t)index*4u)));
        if (offset > offset_offset || (index && offset<(uint32_t)codebook_offsets[index-1]))
            throw Parse_error_str("invalid codebook offset");
        codebook_offsets[index]=(long)offset;
    }
}
