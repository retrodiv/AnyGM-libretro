/* xdelta3 - delta compression tools and library
   ...
   Modified for AnyGM by retrodiv <retrodiv@proton.me>.
   Modifications Copyright (c) 2026 retrodiv <retrodiv@proton.me>.
   Local modifications are MIT-licensed; upstream code remains Apache-2.0.
...
/* AnyGM modification: account for LZMA allocations through the stream hooks,
 * limit decoder memory, and exclude encoder-only initialization. */
...
static void *xd3_lzma_budget_alloc(void *opaque, size_t count, size_t size) {
  xd3_stream *stream = (xd3_stream *)opaque;
  if (size > USIZE_T_MAX || count > USIZE_T_MAX) return NULL;
  return xd3_alloc(stream, (usize_t)count, (usize_t)size);
}

static void xd3_lzma_budget_free(void *opaque, void *address) {
  xd3_free((xd3_stream *)opaque, address);
}

static void xd3_lzma_destroy(xd3_stream *stream, xd3_sec_stream *sec_stream) {
  xd3_lzma_stream *ls = (xd3_lzma_stream *)sec_stream;
  if (ls == NULL) return; /* AnyGM: partially initialized streams are releasable. */
  lzma_end(&ls->lzma);
  xd3_free(stream, ls);
}
...
static int xd3_lzma_init(xd3_stream *stream, xd3_lzma_stream *sec,
  ...
  sec->allocator.alloc = xd3_lzma_budget_alloc;
  sec->allocator.free = xd3_lzma_budget_free;
  sec->allocator.opaque = stream;
  sec->lzma.allocator = &sec->allocator;
  ...
  {
#endif
    ret = lzma_stream_decoder(&sec->lzma, XD3_LZMA_MEMLIMIT, LZMA_TELL_NO_CHECK);
  }
...
static int xd3_decode_lzma(xd3_stream *stream, xd3_lzma_stream *sec,
  ...
  while (1) {
    size_t before_in = sec->lzma.avail_in;
    size_t before_out = sec->lzma.avail_out;
    ...
    case LZMA_OK:
      ...
      if (lret == LZMA_OK && before_in == sec->lzma.avail_in &&
          before_out == sec->lzma.avail_out) {
        stream->msg = "truncated lzma section";
