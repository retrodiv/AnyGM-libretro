...
djw_decode_symbol(xd3_stream *stream, bit_state *bstate, const uint8_t **input,
  ...
  if (base[bits] <= code) {
    ...
    /* AnyGM modification: max_sym is a count, not an inclusive index. */
    if (offset < max_sym) {
...
djw_decode_1_2(xd3_stream *stream, bit_state *bstate, const uint8_t **input,
               ...
               usize_t skip_offset, uint8_t *values, usize_t symbol_count) {
  ...
  while (n < elts) {
    ...
    if (mtf <= RUN_1) {
      /* Repetition. */
      /* AnyGM modification: reject overflowing malformed repeat codes. */
      if (s >= sizeof(usize_t) * 8 || (mtf + 1) > (USIZE_T_MAX >> s)) {
        stream->msg = "secondary decoder repeat overflow";
        return XD3_INVALID_INPUT;
      }
      rep = ((mtf + 1) << s);
      mtf = 0;
      ...
      if (mtf >= symbol_count) {
        stream->msg = "secondary decoder invalid symbol";
        return XD3_INVALID_INPUT;
      }
...
djw_decode_prefix(xd3_stream *stream, bit_state *bstate, const uint8_t **input,
  ...
  return djw_decode_1_2(stream, bstate, input, input_end, cl_inorder, cl_base,
                        ...
                        ALPHABET_SIZE * groups, ALPHABET_SIZE, clen, DJW_TOTAL_CODES);
}
...
static int xd3_decode_huff(xd3_stream *stream, djw_stream *h,
  ...
  /* Outer scope: per-group symbol decoder tables. */
  {
    ...
    {
      /* AnyGM modification: the prefix decoder walks one flat byte array. */
      uint8_t clen[DJW_MAX_GROUPS * ALPHABET_SIZE];
      ...
      for (gp = 0; gp < groups; gp += 1) {
        djw_build_decoder(stream, ALPHABET_SIZE, DJW_MAX_CODELEN, clen + gp * ALPHABET_SIZE,
      ...
      if (groups > 1) {
        ...
        if ((ret =
                 djw_decode_1_2(stream, &bstate, &input, input_end, sel_inorder,
                                ...
                                sel_mtf, sectors, 0, sel_group, groups + 1))) {
          goto fail;
        }
      }
      ...
      /* Now decode each sector. */
      {
        ...
        for (c = 0; c < sectors; c += 1) {
          ...
          if (groups >= 2) {
            ...
            if (gp >= groups) {
              stream->msg = "secondary decoder invalid group";
